...
import requests
import json
from models import db, User, Hello
...
@login_required
def profile():
    res = db.session.query(Hello).all()
    for temp in res:
        print(temp.title)
    return render_template('profile.html', res=res)


@main.route('/recommender') # home page that return 'index'
def recommender():
    suggestions = get_suggestions()
    return render_template('recommender.html', suggestions=suggestions)


@main.route("/watch", methods=['POST'])
@login_required
def watch():
    email = request.form.get('email')
    title = request.form.get('title')

    watchlist = Hello(email=email, title=title)
...
def recommend():
    # getting data from AJAX requestc
